...
class SuperUserCreateTeeSheetSetingsTest(TestCase):
    def setUp(self):
        self.staff_user = User.objects.create_user(
            username="staff_user", password="staff_user_password", is_staff=True
        )
        self.user_profile = UserProfile.objects.create(
            customer_id=7, user=self.staff_user
        )
        self.customer = Customer.objects.create(customer_id=7)
        self.golf_course_group = GolfCourseGroup.objects.create(
            id=1, customer=self.customer
        )
        self.golf_course = GolfCourse.objects.create(
            id=8, customer=self.customer, golf_course_group=self.golf_course_group
        )
    ...
    def test_can_create_tee_sheet_settings(self):
        ...
        tee_sheet_settings = TeeSheetSettings.objects.get(
            number_of_slots=self.data["number_of_slots"]
        )
